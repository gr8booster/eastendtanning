...
import { MessageCircle, X, Send, Loader2, Tag, List as ListIcon, Clipboard, ShoppingCart } from 'lucide-react';
...
export function MaryWellChat() {
  ...
  const showPackages = async () => {
    try {
      ...
      Object.entries(data).forEach(([lvl, info]) => {
        ...
        Object.entries(info.packages).forEach(([type, price]) => { lines.push(`• ${type.replace('_',' ')} — $${price}`); });
        lines.push('');
      });
  ...
  const copyDiscount = async () => {
    ...
    try { await navigator.clipboard.writeText(lastDiscount.code); toast.success('Code copied to clipboard'); }
    catch (e) { toast.error('Failed to copy'); }
  };

  const fetchLotions = async () => {
    try {
      const res = await fetch(`${backendUrl}/api/lotions`);
      const data = await res.json();
      setLotions(Array.isArray(data) ? data : []);
      setLotionsOpen(true);
    } catch (e) { console.error(e); toast.error('Failed to load lotions'); }
  };

  const openCheckoutTanning = () => {
    setCheckoutType('tanning');
    setCheckoutOpen(true);
  };
  const openCheckoutLotion = (l) => {
    setSelectedLotion(l);
    setCheckoutType('lotion');
    setCheckoutOpen(true);
  };
  ...
  const createCheckout = async () => {
    try {
      ...
      if (checkoutType === 'tanning') {
        payload.package_id = `${tanningSelection.level}_${tanningSelection.type}`;
        if (lastDiscount?.code) payload.discount_code = lastDiscount.code;
      } else if (checkoutType === 'lotion') {
        if (!selectedLotion) { toast.error('Select a lotion'); return; }
        payload.lotion_id = selectedLotion.id;
        payload.quantity = 1;
      }
      ...
      if (res.ok && data?.url) {
        ...
        // Fallback: if URL not returned, notify and show success page guidance
  ...
  return (
    <>
      {!isOpen && (
        <Button onClick={handleOpen} className="fixed bottom-6 right-6 h-16 w-16 rounded-full bg-gradient-to-r from-amber-500 to-teal-500 hover:from-amber-600 hover:to-teal-600 shadow-xl z-50" data-testid="mary-well-button">
        ...
        <Card className="fixed bottom-6 right-6 w-96 h-[620px] shadow-2xl z-50 flex flex-col" data-testid="mary-well-chat">
          ...
          <div className="px-4 pt-3 pb-2 border-t border-slate-200 bg-white">
            <div className="flex flex-wrap gap-2">
              ...
              <Button onClick={fetchLotions} size="sm" variant="outline" data-testid="browse-lotions-button">
                <Tag className="h-4 w-4 mr-1" /> Browse Lotions
              </Button>
              <Button onClick={openCheckoutTanning} size="sm" className="bg-secondary text-white" data-testid="checkout-tanning-button">
                <ShoppingCart className="h-4 w-4 mr-1" /> Checkout Tanning
              </Button>
            </div>
          </div>
          ...
          <div className="p-4 border-t border-slate-200 bg-white rounded-b-lg">
            <div className="flex gap-2">
              ...
              <Button onClick={sendMessage} disabled={loading || !inputMessage.trim()} className="bg-gradient-to-r from-amber-500 to-teal-500 hover:from-amber-600 hover:to-teal-600" data-testid="send-button">
